...
def perform_odr(x, y, yerr):
    ...
    myodr = odr.ODR(mydata, linear, beta0=[0,0], maxit=1000000000)
    output = myodr.run()
    return output

files = ["dataset02_lks.ini"]
...
plt.errorbar(sorted_data[0],sorted_data[2],xerr=frequencies_uncerts,yerr=sorted_data[3],marker='x',linestyle="none")
