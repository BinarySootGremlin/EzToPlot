...
files = [["flo_no_capacitor01.ini","flo_capacitor01.ini"],["flo_no_capacitor02.ini","flo_capacitor02.ini"]]
colors = ['r','g','b','purple']
    ...
    for n, file in enumerate(set):
        ...
        with open(file) as f:
            for line in f:
                ...
                voltages.append(clean_values[3]*clean_values[4])
                voltages_uncerts.append(uncert_voltage(clean_values[4]))
        ...
        if(n==0):
            ...
            print("[FLO] Maximale Leistung: %f, Lastwiderstand: %f" % (watt,res)) 
    plt.legend()
    plt.grid(linestyle='dotted')
    plt.savefig("Data" + str(number) + "_flo.png")
    plt.show()

text_file = open("flo_data.txt", "w")
