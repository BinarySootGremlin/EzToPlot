...
def get_distance():
    dist_file = open("flo_distance01.ini","r")
...
def prop(x,y,u_x,u_y):
   return u_x
filename = "flo_task01.ini"
...
plotter.redefine_y(filename,filename,2,my,prop)
plotter.describe()
#plotter.err_plot("test_values.ini","Messung","Beugungsordnung","Abstand")

plotter.fit_plot(filename, linear_func, 2, colors=["r"],override_start=[[1,0]],legends_fit=["Fitfunktion"],title="$Messung_{flo}$",x_label="Beugungsmaximum #",y_label="Abstand zur\n 0. Ordnung\n in m",show=False)
plt.ylim(0, 0.3)
plt.xlim(0, 6)
plt.gcf().set_dpi(280)
