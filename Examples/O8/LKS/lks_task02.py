...
def get_distance():
    dist_file = open("lks_distance02.ini","r")
...
filename = "lks_task02.ini"
plotter = ez.ez_parser()
plotter.setup_file(filename,x_column=0,y_column=1,y_err_types=["plain","percentage"],y_err_setting=[0.07,1])
plotter.stretch_y(filename,0.005)
plotter.describe()
#plotter.err_plot("test_values.ini","Messung","Beugungsordnung","Abstand")

plotter.fit_plot(filename, linear_func, 1, colors=["r"],override_start=[[1]],legends_fit=["Fitfunktion"],title="$Messung_{lks}$",x_label="Beugungsminimum #",y_label="Abstand zur\n 0. Ordnung\n in m",show=False)
plt.ylim(0, 0.05)
plt.xlim(0, 13)
plt.gcf().set_dpi(280)
